...
local function create_test_buffer()
  ...
  vim.api.nvim_buf_set_name(bufnr, "/tmp/pi-editor-" .. test_counter .. ".pi.md")
...
local function reset_module(fixtures)
  package.loaded["local_plugins.blink-pi"] = nil
  local blink_pi = require "local_plugins.blink-pi"
  blink_pi.configure { home_dir = fixtures.home }
  blink_pi.reset_cache()
  blink_pi.configure { nested_placeholders_supported = nil }
  ---@diagnostic disable-next-line: missing-parameter
  return blink_pi.new()
end

local function get_items(source, ctx)
  local result
...
function M.test_settings_skills()
  ...
  assert_true(
    find_item(items, "/skill:claude-project-skill") ~= nil,
    "Project settings skill found via relative path"
  )
...
function M.test_filename_filtering()
  ...
  test_counter = test_counter + 1
  local bufnr3 = vim.api.nvim_create_buf(false, false)
  vim.api.nvim_buf_set_name(bufnr3, "/tmp/claude-prompt-" .. test_counter .. ".md")
  vim.api.nvim_set_current_buf(bufnr3)
  vim.bo[bufnr3].filetype = "markdown"
