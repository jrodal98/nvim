...
local function setup_fixtures()
   ...
   vim.fn.writefile({ vim.json.encode(plugins_json) }, home_claude .. "/plugins/installed_plugins.json")
...
local function create_test_buffer()
   return harness.create_test_buffer "claude-prompt-test-"
end

local function reset_module(fixtures)
   return harness.reset_module("local_plugins.blink-claude", fixtures)
end
         ...
         if item.label == "/with-args" then
            ...
            if has_luasnip then
               ...
               assert_contains(
                  item.insertText,
                  "${2:--option ${3:VALUE}}",
                  "Nested snippet for optional flag with value"
               )
            else
      ...
      for _, item in ipairs(items) do
         if item.label == "/nested-args" then
            ...
            if has_luasnip then
               ...
               assert_contains(
                  item.insertText,
                  "${4:--completion-promise ${5:TEXT}}",
                  "Nested snippet for --completion-promise TEXT"
               )
               ...
               assert_contains(
                  ...
                  "Simple placeholder for --completion-promise TEXT"
               )
...
function M.run_all_tests()
   harness.run("BLINK-CLAUDE TEST SUITE", {
      M.test_yaml_parsing,
