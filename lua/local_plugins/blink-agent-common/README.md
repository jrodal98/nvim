...
Owns: session-scoped caching with pcall/notify error handling, target-buffer detection (markdown + basename pattern), slash-context trigger logic (`/` at line start or after whitespace, continuing through `[%w:_-]`), the provider protocol (`new`, `get_trigger_characters` → `{"/", ":"}`, `get_completions`), and test hooks (`configure`, `reset_cache`).
...
No dedicated suite; both consumers exercise every module:
